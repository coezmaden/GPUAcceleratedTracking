...
function gen_signal(
    ...
    sampling_frequency = num_samples / duration |> Hz
    signal = gen_blank_signal(system, num_samples, num_ants, prn)
    signal = gen_signal!(
        signal,
        system,
        prn, 
...
# CPU Matrix multiple prns
function gen_signal!(
    signal::StructArray{Complex{Float32}, 3, NamedTuple{(:re, :im), Tuple{T, T}}, Int64},
    system,
    prns, 
    carrier_frequency,
    num_samples,
    ...
    start_code_phase,
    start_carrier_phase
) where {N, T <: Array{Float32, 3}}
    code_phases = get_code_frequency(system) / sampling_frequency .* (0:num_samples-1) .+ start_code_phase
    upsampled_codes = system.codes[1 .+ mod.(floor.(Int, code_phases), get_code_length(system)), prns]
    carrier_phases = Vector{Float32}(2π * (0:num_samples-1) * carrier_frequency / sampling_frequency .+ start_carrier_phase)
    signal.re[1:num_samples,:] = cos.(carrier_phases) .* upsampled_codes * ones(N)'
    signal.im[1:num_samples,:] = sin.(carrier_phases) .* upsampled_codes * ones(N)'
    return signal
...
# GPU Matrix single antenna multiple sats
function gen_signal!(
    signal::StructArray{Complex{Float32}, 3, NamedTuple{(:re, :im), Tuple{T, T}}, Int64},
    system,
    prn, 
...
function gen_signal!(
    ...
    start_code_phase,
    start_carrier_phase
) where {N, T <: CuArray{Float32, 3, CUDA.Mem.DeviceBuffer}}
    code_phases = get_code_frequency(system) / sampling_frequency .* (0:num_samples-1) .+ start_code_phase
    upsampled_codes = system.codes[1 .+ mod.(floor.(Int, code_phases), get_code_length(system)), prns]
    carrier_phases = CuVector{Float32}(2π * (0:num_samples-1) * carrier_frequency / sampling_frequency .+ start_carrier_phase)
    for svidx = 1:length(prns)
        signal.re[1:num_samples, :, svidx] = cos.(carrier_phases) .* upsampled_codes[:, svidx] * CUDA.ones(N)'
        signal.im[1:num_samples, :, svidx] = sin.(carrier_phases) .* upsampled_codes[:, svidx] * CUDA.ones(N)'
    end
...
@inline gen_blank_signal(system::S, num_samples, num_ants::NumAnts{1}, prn) where {CO <: Matrix, S <: AbstractGNSS{CO}} = StructArray{ComplexF32}(undef, num_samples, length(prn))
...
@inline gen_blank_signal(system::S, num_samples, num_ants::NumAnts{N}, prn) where {CO <: Matrix, S <: AbstractGNSS{CO}, N} = StructArray{ComplexF32}(undef, num_samples, N, length(prn))
...
@inline gen_blank_signal(system::S, num_samples, num_ants::NumAnts{1}, prn) where {CO <: CuMatrix, S <: AbstractGNSS{CO}} = StructArray{ComplexF32}((CuArray{Float32}(undef, num_samples, length(prn)), CuArray{Float32}(undef, num_samples, length(prn))))
...
@inline gen_blank_signal(system::S, num_samples, num_ants::NumAnts{N}, prn) where {CO <: CuMatrix, S <: AbstractGNSS{CO}, N} = StructArray{ComplexF32}((CuArray{Float32}(undef, num_samples, N, length(prn)), CuArray{Float32}(undef, num_samples, N, length(prn))))