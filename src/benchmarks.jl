...
function _run_kernel_benchmark(
    ...
    @belapsed CUDA.@sync $kernel_algorithm(
        $threads_per_block,
        $blocks_per_grid,
        $shmem_size,
        $nothing,
...
function _run_kernel_benchmark(
    ...
    @belapsed CUDA.@sync @cuda threads=$threads_per_block blocks=cld($blocks_per_grid,2) shmem=$shmem_size $downconvert_and_correlate_kernel_5431!(
        $accum.re,
        $accum.im,
        $carrier_replica.re,
        $carrier_replica.im,
...
function _bench_pure_reduction(num_samples, num_ants, num_correlators)
    ...
    return @belapsed CUDA.@sync begin
