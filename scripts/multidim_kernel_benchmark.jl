...
for num_samples in N 
    ...
    CUDA.@profile begin
        NVTX.@range "Multidim correlate kernel N=$(num_samples)" begin
            NVTX.@range "gen_code_replica_texture_mem_strided_nsat_kernel! N=$(num_samples)" begin
                @cuda threads=threads_per_block_1 blocks=(blocks_per_grid_1, num_sats) gen_code_replica_texture_mem_strided_nsat_kernel!(
                    code_replica,
                    codes,
                    code_frequency,
            ...
            NVTX.@range "downconvert_and_correlate_kernel_3d_4431! N=$(num_samples)" begin
                @cuda threads=threads_per_block blocks=blocks_per_grid shmem=shmem_size downconvert_and_correlate_kernel_3d_4431!(
                    accum.re,
                    accum.im,
                    signal.re,
